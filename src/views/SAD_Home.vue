  ...
  <div class="section-wrapper row bg-light align-items-center justify-content-center flex-fill" v-if="loaded">
    <section class="block block__form">
      ...
      <form class="text-left pt-5" v-on:submit.prevent="submitForm">
        ...
        <fieldset class="form-group">
          <div class="row">
            <legend class="col-form-label col-sm-4 pt-0">Experiment Order</legend>

            <div class="col-sm-8">
              <div class="form-check">
                <input type="radio" class="form-check-input" id="order_1" value="CUE_MAT" v-model="experimentSettings.order">
                <label class="form-check-label" for="order_1">Cue &#8594; MAT</label>
              </div>

              <div class="form-check">
                <input type="radio" class="form-check-input" id="order_2" value="MAT_CUE" v-model="experimentSettings.order">
                <label class="form-check-label" for="order_2">MAT &#8594; Cue</label>
              </div>
            </div>
          </div>
        </fieldset>
        ...
        <fieldset class="form-group">
          <div class="row">
            <legend class="col-form-label col-sm-4 pt-0">With validation task</legend>
            <div class="col-sm-8">
              <div class="form-check">
                <input type="checkbox" class="form-check-input" id="with_validation" v-model="experimentSettings.withValidationTask">
              </div>
            </div>
          </div>
        </fieldset>
...
export default {
    ...
    submitForm() {
      axios
        .post("/experiment/sad/create", {
          ...
          with_validation: this.experimentSettings.withValidationTask
        })
        .then(({ data }) => {
          //Perform Success Action
          console.log("success -> go to next", data);
          if (data.isOk) {
            this.$router.push(`/experiment/sad/${data.experimentId}`);
          }
        })
        .catch(() => {
  ...
  mounted() {
    axios.post("/experiment/sad/request-new-id").then(({ data }) => {
