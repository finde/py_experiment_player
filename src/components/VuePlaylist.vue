...
export default {
  ...
  mounted: function () {
    ...
    self.currentPlayer.addEventListener("timeupdate", function () {
      ...
      var value =
        (100 / self.getTotalDuration(self.playlist)) *
  ...
  methods: {
    ...
    switchVideo() {
      if (this.playerIndex < this.playlist.length - 1) {
        ...
        self.currentPlayer.addEventListener("timeupdate", function () {
          ...
          var value =
            (100 / self.getTotalDuration(self.playlist)) *
            (self.playlist[self.playerIndex].timeInPlaylist[0] +
              self.currentPlayer.currentTime);
          // // console.log(value + "% / 100%")
          // Update the slider value
          seekBar.value = value;
    ...
    checkSeekBarVideo(seekValue, index /*, time*/) {
      ...
      if (
        seekValue <
          (this.playlist[index].timeInPlaylist[1] /
            this.getTotalDuration(this.playlist)) *
            100 &&
        seekValue >
          (this.playlist[index].timeInPlaylist[0] /
            this.getTotalDuration(this.playlist)) *
            100
      ) {
        ...
        this.currentPlayer.currentTime =
          this.getTotalDuration(this.playlist) * (seekValue / 100) -
          this.playlist[index].timeInPlaylist[0];
        ...
        for (var i = 0; i < this.playlist.length; i++) {
          if (
            seekTo < this.playlist[i].timeInPlaylist[1] &&
            seekTo > this.playlist[i].timeInPlaylist[0]
          ) {
